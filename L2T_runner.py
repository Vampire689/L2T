...
def main():
    ...
    parser.add_argument('--batch_size', type=int, help='batch size / 2 for how many domain computations in parallel', default=16)
    ...
    parser.add_argument('--max_solver_batch', type=float, default=2000, help='max batch size for bounding computations')
    ...
    if args.data=='mnist' or args.data=='cifar10':
        csvfile = open('./cifardata/%s_test.csv'%(args.data), 'r')
    ...
    for new_idx, idx in enum_batch_ids:
        ...
        if args.record:
            graph_df = pd.read_pickle(record_name)
            if not pd.isna(graph_df.loc[new_idx]['Idx']):
