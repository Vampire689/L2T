...
models = {}
models['cifar_base_expnn'] = 'nn_branching/nn_models/cifar/expnn_53.5_rnn_32_relu.pt'
models['mnist_base_expnn'] = 'nn_branching/nn_models/cifar/expnn_53.5_rnn_32_relu.pt'
...
def bab(gt_prop, verif_layers, domain, return_dict, timeout, batch_size, method, tot_iter,  parent_init,
    ...
    if args.branching_choice == 'heuristic':
        ...
        if method == 'prox':
            if 'cifar' in args.data:
                ...
                branching_net_name = models['mnist_base_expnn']
            else: 
    ...
    with torch.no_grad():
        ...
        min_lb, min_ub, ub_point, nb_states, fail_safe_ratio = relu_bab(intermediate_net, bounds_net, branching_net_name, domain, decision_bound,
                                                        ...
                                                        parent_init_flag=parent_init, gurobi_specs=gurobi_dict, branching_threshold=pref_branching_thd)
        # except:
        #     # del domains
        #     min_lb, min_ub, ub_point, nb_states, fail_safe_ratio = None, None, None, None, None
...
def main():
    # para for 2_255_cifar
    parser = argparse.ArgumentParser()
    parser.add_argument('--record', action='store_true', help='file to save results', default=True)
    ...
    parser.add_argument('--nn_name', type=str, help='network architecture name', default="mnist_0.3")
    parser.add_argument('--data', type=str, default='mnist')
    parser.add_argument('--testing', action='store_true')
    parser.add_argument('--batch_size', type=int, help='batch size / 2 for how many domain computations in parallel', default=32)
    ...
    parser.add_argument('--branching_choice', type=str, choices=['heuristic', 'nn'], help='type of branching choice used', default='nn')
    parser.add_argument('--tot_iter', type=float, help='how many total iters to use for the method', default=100)
    parser.add_argument('--max_solver_batch', type=float, default=1800, help='max batch size for bounding computations')
    ...
    parser.add_argument('--feta', type=float, default=1e1)
    ...
    # parser.add_argument('--testing', action='store_true')
    # parser.add_argument('--batch_size', type=int, help='batch size / 2 for how many domain computations in parallel', default=150)
    # parser.add_argument('--gurobi_p', type=int, help='number of threads to use in parallelizing gurobi over domains', default=1)
    # parser.add_argument('--method', type=str, choices=["prox", "adam", "gurobi", "gurobi-anderson", "dj-adam"], help='method to employ for bounds', default="prox")
    # parser.add_argument('--branching_choice', type=str, choices=['heuristic', 'nn'], help='type of branching choice used', default='nn')
    # parser.add_argument('--tot_iter', type=float, help='how many total iters to use for the method', default=100)
    ...
    if args.record:
        if args.record_name is not None:
            ...
            if args.branching_choice=='heuristic':
                ...
                method_name += f'NN_{algorithm_name}{parent_init}{algo_string}'
                columns += [f'BSAT_NN_{algorithm_name}', f'BBran_NN_{algorithm_name}', f'BTime_NN_{algorithm_name}', f'FS_Ratio_{algorithm_name}']
    ...
    for new_idx, idx in enum_batch_ids:
        ...
        if args.data == 'cifar':
            ...
            x, verif_layers, test, domain = load_1to1_eth(args.data, args.nn_name, idx=imag_idx, test=tests, eps_temp=eps_temp,
                                                  max_solver_batch=args.max_solver_batch, relax=relax)
            if x is None:
        ...
        if args.record:
            ...
            if args.branching_choice == "heuristic":
                ...
                graph_df.loc[new_idx][f"BSAT_NN_{algorithm_name}"] = bab_out
                graph_df.loc[new_idx][f"BBran_NN_{algorithm_name}"] = bab_nb_states
                graph_df.loc[new_idx][f"BTime_NN_{algorithm_name}"] = bab_time
