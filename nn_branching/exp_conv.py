    ...
    def forward(self, lower_bounds_all, upper_bounds_all, dual_vars, primals, primal_inputs, layers, mu):
        ...
        for layer_idx, layer in enumerate(layers['fixed_layers']):
            ...
            # node features
                ...
                layer_nb_embeddings_temp = F.relu(self.fc3(nb_embeddings_input))
                layer_nb_embeddings = th.cat([layer_nb_embeddings_temp, mu[relu_count_idx+1].reshape(layer_nb_embeddings_temp.size())], dim=1)
                layer_nb_embeddings = self.fc3_2(layer_nb_embeddings)
...
class ExpNet(nn.Module):
    ...
    def forward(self, lower_bounds_all, upper_bounds_all, dual_vars, primals, primal_inputs, layers, masks, pre_mu=None):
        mu = self.EmbedUpdates(lower_bounds_all, upper_bounds_all, dual_vars, primals, primal_inputs, layers, pre_mu=pre_mu)
