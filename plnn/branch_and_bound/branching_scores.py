...
class BranchingChoice:
    ...
    def __init__(self, init_mask, sparsest_layer, net_weights, branching_net_name, lr=1e-4, wd=1e-4, online=False, branch_net=None):
        ...
        if not branch_net:
            if branching_net_name is None:
                ...
                print('Using nn branching choices')
                model = ExpNet(32)
            ...
            self.model = branch_net.model
            self.optimizer = branch_net.optimizer
            self.criterion = branch_net.criterion
            trans_len = []
            temp = 0
            for i in init_mask:
                temp += len(i.view(-1))
    ...
    def nn_branching_decision(self, lower_bounds_stacks, upper_bounds_stacks, dual_vars_stacks, primal_input_stacks, primals_stacks, orig_mask_stacks, features=None):
        batch_size = len(lower_bounds_stacks)
        mask_temp = list(zip(*orig_mask_stacks))
        mask = [torch.stack(i,0) for i in mask_temp]
        mask = [(i == -1).float().view(batch_size, -1) for i in mask]
        ...
        if not features:    pre_mu = []
        else:
            features_temp = list(zip(*features))
            pre_mu = [torch.cat(i, 0).cuda() for i in features_temp]

        start = time.time()

        scores, mu = self.model(lower_bounds, upper_bounds, dual_vars, primals, primal_inputs, self.layers, mask_1d, pre_mu=pre_mu)
        end = time.time()
        print(f'ExpNet requires: {end-start}')
            
        decision_list = []
        for s_idx in range(len(scores)):
            nn_score, choice = torch.max(scores[s_idx],0)
        ...
        return decision_list, mu

    
    def nn_branching_decision_all(self, lower_bounds_stacks, upper_bounds_stacks, dual_vars_stacks, primal_input_stacks, primals_stacks, orig_mask_stacks, generate_mode=True, features=None):
        batch_size = len(lower_bounds_stacks)
        mask_temp = list(zip(*orig_mask_stacks))
        mask = [torch.stack(i,0) for i in mask_temp]
        mask = [(i == -1).float().view(batch_size, -1) for i in mask]
        ...
        decision_list = []
        if generate_mode:
            # [len(all_scores), batch_idx, score]
            min_scores_len = min([len(one_batch_scores) for one_batch_scores in scores])
            for choice in range(min_scores_len):
                decisions = []
                for s_idx in range(len(scores)):
                    idx = (mask_1d[s_idx].nonzero()[choice]).item()
                    dec_lay =  ((self.trans_len>idx).nonzero()[0]).item()
                    if dec_lay == 0:
                        ...
                        dec_idx = (idx - self.trans_len[dec_lay-1]).item()
                    decisions.append([dec_lay, dec_idx])
                decision_list.append(decisions)

            return scores, decision_list
        else:
            return scores, mu
    ...
    def online_learning(self, kw_decision, improvement):
        ...
        loss = self.nn_score - kw_score + improvement
        loss.backward()
        self.optimizer.step()
        self.model.eval()
