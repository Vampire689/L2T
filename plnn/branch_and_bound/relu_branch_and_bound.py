...
from tools.save_read_data import save_obj, read_obj


class TrainData:
    def __init__(self, orig_lb_stacks_current, orig_ub_stacks_current, orig_parent_sol_stacks_current, orig_parent_ub_points_stacks_current, orig_gl_lb_stacks_current, orig_mask_stacks_current, parent_batch_idx_stacks, improvement=None):
        self.orig_lb_stacks_current = copy.deepcopy(orig_lb_stacks_current)
        self.orig_ub_stacks_current = copy.deepcopy(orig_ub_stacks_current)
        self.orig_parent_sol_stacks_current = copy.deepcopy(orig_parent_sol_stacks_current)
        self.orig_parent_ub_points_stacks_current = copy.deepcopy(orig_parent_ub_points_stacks_current)
        self.orig_gl_lb_stacks_current = copy.deepcopy(orig_gl_lb_stacks_current)
        self.orig_mask_stacks_current = copy.deepcopy(orig_mask_stacks_current)
        self.parent_batch_idx_stacks = copy.deepcopy(parent_batch_idx_stacks)
        self.improvement = copy.deepcopy(improvement)
...
class ReLUDomain:
    ...
    def __init__(self, mask, lb=-float('inf'), ub=float('inf'), lb_all=None, up_all=None, parent_solution=None,
                 parent_ub_point=None, feature=None, parent_batch_idx = None):
        self.mask = mask
        self.lower_bound = lb
        self.upper_bound = ub
        ...
        self.parent_batch_idx = parent_batch_idx
...
def relu_bab(intermediate_net, bounds_net, branching_net_name, domain, decision_bound, eps=1e-4, sparsest_layer=0,
             ...
             gurobi_specs=None, branching_threshold=0.2, feature_mode=True):
    '''
    ...
    if branching_net_name is None:
        ...
        ## can switch back to nn decisions
        nn_improvements_stacks_previous = []
        dom_ub_nn=[]; dom_lb_nn=[]; dom_ub_point_nn=[] 
        updated_mask_nn=[]; dom_lb_all_nn=[]; dom_ub_all_nn=[]
        dual_solutions_nn =[]
        results_nn_features = []
        # performance evaluation
        heuristic_total = 0; nn_total = 0
    ...
    while global_ub - global_lb > eps:
        ...
        for batch_idx in range(effective_batch_size):
            ...
            feature = candidate_domain.feature
            ...
            # collect more information if nn branching decision is used
            if not heuristic_choice:
        ...
        if heuristic_choice:
            ...
            if effective_batch_size > 0:
                if feature_mode:
                    branching_decision_list_nn, features = branching_tools.nn_branching_decision(orig_lb_stacks_current, orig_ub_stacks_current, orig_parent_sol_stacks_current, orig_parent_ub_points_stacks_current, orig_gl_lb_stacks_current, orig_mask_stacks_current, features)
                else:
                    branching_decision_list_nn, _ = branching_tools.nn_branching_decision(orig_lb_stacks_current, orig_ub_stacks_current, orig_parent_sol_stacks_current, orig_parent_ub_points_stacks_current, orig_gl_lb_stacks_current, orig_mask_stacks_current, features)
                branching_decision_list += branching_decision_list_nn
        ...
        if not heuristic_choice:
            ...
            results_final_features = []
            results_nn = [dom_ub_nn, dom_lb_nn, dom_ub_point_nn]
            results_nn_lists = [updated_mask_nn, dom_lb_all_nn, dom_ub_all_nn, dual_solutions_nn]
            ...
            results_temp_features = features if features else []
            ...
            for batch_idx_prev in range(previous_chunk_size):
                ...
                nn_improvement = nn_improvements_stacks_previous[batch_idx_prev]
                if nn_improvement >= heuristic_improvement:
                    # use previous nn results
                    nn_total += 1
                    bab.add_terms(results_final, results_nn, batch_idx_prev)
                    bab.add_terms_lists(results_final_lists, results_nn_lists, batch_idx_prev)
                else:
            ...
            results_final_features += list(results_nn_features)
            ...
            nn_improvements_stacks_previous=[]
            parent_init_stacks_previous = None
            for item in results_nn: item[:] = []
            for item in results_nn_lists: item[:] = []
            results_nn_features = []
            ...
            for batch_idx in range(previous_chunk_size, effective_batch_size):
                nn_improvement = ((min(dom_lb_temp[2*batch_idx],0) + min(dom_lb_temp[2*batch_idx+1],0)) - 2*domain_lower_bounds_stacks[batch_idx][0])/(-2*domain_lower_bounds_stacks[batch_idx][0])
                imp_ls += nn_improvement.tolist()
                if nn_improvement < branching_threshold:
                    # nn branching decision is not satisfactory
                    ...
                    nn_improvements_stacks_previous.append(nn_improvement)
                    ...
                    # switch back to nn decisions when heuristic choices are worse
                    bab.add_terms(results_nn, results_temp, batch_idx)
                    bab.add_terms_lists(results_nn_lists, results_temp_lists, batch_idx)
                    bab.add_terms_features(results_nn_features, results_temp_features, batch_idx - previous_chunk_size)
                else:
                    # nn performance is satisfactory 
                    nn_total += 1
                    bab.add_terms(results_final, results_temp, batch_idx)
                    bab.add_terms_lists(results_final_lists, results_temp_lists, batch_idx)
                    bab.add_terms_features(results_final_features, results_temp_features, batch_idx - previous_chunk_size)
            print(imp_ls)
        ...
        if not heuristic_choice:
            if len(dom_ub) == 0:
                ...
                dom_ub, dom_lb, dom_ub_point, updated_mask, dom_lb_all, dom_ub_all, dual_solutions, dom_features = bab.modify_forms(results_final, results_final_lists, results_final_features)
        else:
        ...
        for batch_idx in range(dom_lb.shape[0]):
            if dom_lb[batch_idx] == float('inf') or dom_ub[batch_idx] == float('inf') or \
                ...
                if not heuristic_choice:
                    ...
                    if dom_features and feature_mode:
                        dom_to_add.feature = [dom_feature.unsqueeze(0).cpu() for dom_feature in dom_features[batch_idx//2]]
    ...
    if not heuristic_choice:
        fail_safe_ratio = heuristic_total/(nn_total+heuristic_total)

    return global_lb, global_ub, global_ub_point, nb_visited_states, fail_safe_ratio


def generate_relu_bab(intermediate_net, bounds_net, branching_net_name, imag_idx, domain, decision_bound, eps=1e-4, sparsest_layer=0,
             timeout=float("inf"), batch_size=5, max_mem_consumption=100, parent_init_flag=True, 
             gurobi_specs=None, branching_threshold=0.2, branch_net=None, train_data_path='cifardata/train_data/'):
    '''
    Uses branch and bound algorithm to evaluate the global minimum
    of a given neural network. Splits according to KW.
    Does ReLU activation splitting (not domain splitting, the domain will remain the same throughout)
    ...
    Returns         : Lower bound and Upper bound on the global minimum,
                      as well as the point where the upper bound is achieved
    '''
    train_data_path = 'cifardata/train_data/'
    # train_data_path = 'cifardata/test_data/'
    try:
        dic = read_obj(train_data_path + str(imag_idx))
    except:
        dic = {}
    ...
    # initialise branching related terms
    branching_tools = BranchingChoice(updated_mask, sparsest_layer, intermediate_net.weights, branching_net_name, branch_net=branch_net)
    ...
    while global_ub - global_lb > eps:
        ...
        for batch_idx in range(effective_batch_size):
            ...
            orig_lbs = candidate_domain.lower_all
            orig_ubs = candidate_domain.upper_all
            parent_batch_idx = candidate_domain.parent_batch_idx

            # collect branching related information
            orig_lb_stacks_current.append(orig_lbs)
            orig_ub_stacks_current.append(orig_ubs)
            orig_mask_stacks_current.append(mask)
            if parent_batch_idx != None:
                parent_batch_idx_stacks.append(parent_batch_idx)
        ...
        heu_branching_decision_list, _ = branching_tools.heuristic_branching_decision(orig_lb_stacks_current, orig_ub_stacks_current, orig_mask_stacks_current)
        if while_time in dic:
            print("img_idx:", imag_idx, 'kw times:', while_time, 'is ok')
            data = dic[while_time]
            scores, branching_decision_lists = branching_tools.nn_branching_decision_all(data.orig_lb_stacks_current, data.orig_ub_stacks_current, data.orig_parent_sol_stacks_current, data.orig_parent_ub_points_stacks_current, data.orig_gl_lb_stacks_current, data.orig_mask_stacks_current)
            nn_improvement = data.improvement
            # print(data.improvement)
            # print("__________________________________________________________________________________")
        else:
            nn_improvement = [[] for _ in range(effective_batch_size)]
            print(parent_batch_idx_stacks)
            train_data = TrainData(orig_lb_stacks_current, orig_ub_stacks_current, orig_parent_sol_stacks_current, orig_parent_ub_points_stacks_current, orig_gl_lb_stacks_current, orig_mask_stacks_current, parent_batch_idx_stacks)
            # Compute branching choices
            # TODO: branching will return IndexError in case no ambiguous ReLU is left. Should catch this, and compute
            if effective_batch_size > 0:
                scores, branching_decision_lists = branching_tools.nn_branching_decision_all(orig_lb_stacks_current, orig_ub_stacks_current, orig_parent_sol_stacks_current, orig_parent_ub_points_stacks_current, orig_gl_lb_stacks_current, orig_mask_stacks_current)
            
            
            for idx_decsion, branching_decision_list in enumerate(branching_decision_lists):
                for batch_idx, branching_decision in enumerate(branching_decision_list):
                    branching_layer_log[branching_decision[0]] |= {2*batch_idx, 2*batch_idx+1}
                    orig_lbs  =  orig_lb_stacks_current[batch_idx]
                    orig_ubs  =  orig_ub_stacks_current[batch_idx]
                ...
                for batch_idx in range(effective_batch_size):
                    nn_improvement[batch_idx].append( ((min(dom_lb_temp[2*batch_idx],0) + min(dom_lb_temp[2*batch_idx+1],0)) - 2*domain_lower_bounds_stacks[batch_idx][0])/(-2*domain_lower_bounds_stacks[batch_idx][0]) )
                if idx_decsion % 100 == 0:    
                    print("img_idx:", imag_idx, 'kw times:', while_time, 'batch:', effective_batch_size, 'decision:', idx_decsion, 'generate success')

            nn_improvement = torch.tensor(nn_improvement).cuda()
            train_data.improvement = copy.deepcopy(nn_improvement)
            dic[while_time] = train_data
            save_obj(dic, train_data_path + str(imag_idx))


        heuristic_choice = True
        gt_decision_list = []
        for batch_idx in range(effective_batch_size):
            decision_idx = torch.argmax(nn_improvement[batch_idx])
            gt_decision_list.append(branching_decision_lists[decision_idx][batch_idx])
        for batch_idx, branching_decision in enumerate(gt_decision_list):
            branching_layer_log[branching_decision[0]] |= {2*batch_idx, 2*batch_idx+1}
            orig_lbs  =  orig_lb_stacks_current[batch_idx]
            orig_ubs  =  orig_ub_stacks_current[batch_idx]
        ...
        for batch_idx in range(effective_batch_size):
            print('practical improvement:', ((min(dom_lb_temp[2*batch_idx],0) + min(dom_lb_temp[2*batch_idx+1],0)) - 2*domain_lower_bounds_stacks[batch_idx][0])/(-2*domain_lower_bounds_stacks[batch_idx][0]) )
        ...
        for batch_idx in range(dom_lb.shape[0]):
            ...
            if dom_lb[batch_idx] == float('inf') or dom_ub[batch_idx] == float('inf') or \
                ...
                if add_count < batch_size:
                    ...
                    dom_to_add = ReLUDomain(
                        c_updated_mask, lb=dom_lb[batch_idx].unsqueeze(0), ub=dom_ub[batch_idx].unsqueeze(0),
                        lb_all=c_dom_lb_all, up_all=c_dom_ub_all, parent_solution=c_dual_solutions, parent_batch_idx = batch_idx // 2
                    ).to_cpu()
                    dom_to_add.parent_ub_point = dom_ub_point[batch_idx].cpu()
                    bab.add_domain(dom_to_add, domains)
                add_count += 1
