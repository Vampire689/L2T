...
class TrainData:
    def __init__(self, orig_lb_stacks_current, orig_ub_stacks_current, orig_parent_sol_stacks_current, orig_parent_ub_points_stacks_current, orig_gl_lb_stacks_current, orig_mask_stacks_current, improvement=None):
...
class ReLUDomain:
    ...
    def __init__(self, mask, lb=-float('inf'), ub=float('inf'), lb_all=None, up_all=None, parent_solution=None,
                 parent_ub_point=None, feature=None):
        self.mask = mask
        self.lower_bound = lb
        self.upper_bound = ub
...
def relu_bab(intermediate_net, bounds_net, branching_net_name, domain, decision_bound, eps=1e-4, sparsest_layer=0,
             ...
             gurobi_specs=None, branching_threshold=0.2, feature_mode=False):
    '''
...
def generate_relu_bab(intermediate_net, bounds_net, branching_net_name, imag_idx, domain, decision_bound, eps=1e-4, sparsest_layer=0,
    ...
    while global_ub - global_lb > eps:
        ...
        if while_time in dic:
            ...
            train_data = TrainData(orig_lb_stacks_current, orig_ub_stacks_current, orig_parent_sol_stacks_current, orig_parent_ub_points_stacks_current, orig_gl_lb_stacks_current, orig_mask_stacks_current)
            # Compute branching choices
            # TODO: branching will return IndexError in case no ambiguous ReLU is left. Should catch this, and compute
            if effective_batch_size > 0:
