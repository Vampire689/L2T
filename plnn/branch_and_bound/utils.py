...
def add_terms_features(dest, orig, batch_idx):
    if orig:
        dest.append([torch.zeros(1,1,device='cuda')] + [item[batch_idx].clone().detach() for item in orig[1:]])
        # dest.append([item[batch_idx].clone().detach() for item in orig])


def modify_forms(items, lists, features=None):
    ...
    final_results.append(features if features else []) 
