...
def load_mnist_1to1_exp(dataset, model, idx, test = None, mnist_test = None, eps_temp=None, lb_abs=-1000,
                  max_solver_batch=10000, relax=False):
    device = 'cpu'
    if model=='mnist_0.1':
    ...
    normalize = transforms.Normalize(mean=[0.1307, ],
                                         std=[0.3081, ])
    mnist_test = datasets.MNIST('./mnistdata/', train=False, download=True,
    ...
    if  y_pred != y: 
        ...
        if dataset == 'mnist':
            x_m_eps = ((x - eps_temp).clamp(0,1).reshape([1, 1, 28, 28]) - 0.1307) / 0.3081
            x_p_eps = ((x + eps_temp).clamp(0,1).reshape([1, 1, 28, 28]) - 0.1307) / 0.3081
        ...
        if not relax:
            ...
            added_prop_layers = new_add_properties(model, y, lb_abs, domain=domain, max_solver_batch=max_solver_batch)
...
def new_max_pool(candi_tot):
    ...
    while candi_tot > 1:
        ...
        for idl in even:
            weight_mp_1[idl, idl+1] = -1
            weight_mp_1[idl+1, idl] = 1
        max_pool_layer1.weight = Parameter(weight_mp_1, requires_grad=False)
        max_pool_layer1.bias = Parameter(bias_mp_1, requires_grad=False)
        layers.append(max_pool_layer1)

        max_pool_layer2 = nn.Linear(candi_tot, 2*candi_tot, bias=True)
        weight_mp_2 = torch.zeros(2*candi_tot, candi_tot)
        bias_mp_2 = torch.zeros(2*candi_tot)
        for idl in range(candi_tot):
            weight_mp_2[2*idl, idl] = -1
            weight_mp_2[2*idl+1, idl] = 1
        max_pool_layer2.weight = Parameter(weight_mp_2, requires_grad=False)
        max_pool_layer2.bias = Parameter(bias_mp_2, requires_grad=False)
        layers.append(max_pool_layer2)
        layers.append(nn.ReLU())

        max_pool_layer3 = nn.Linear(2*candi_tot, candi_tot, bias=True)
        weight_mp_3 = torch.zeros(candi_tot, 2*candi_tot)
        bias_mp_3 = torch.zeros(candi_tot)
        for idl in range(candi_tot):
            weight_mp_3[idl, 2*idl] = 1
            weight_mp_3[idl, 2*idl+1] = 1
        if candi_tot % 2 == 1:  weight_mp_3[-1, -2] = -1
        max_pool_layer3.weight = Parameter(weight_mp_3, requires_grad=False)
        max_pool_layer3.bias = Parameter(bias_mp_3, requires_grad=False)
        layers.append(max_pool_layer3)
        ...
        sum_layer_bias = torch.zeros(new_candi_tot)
        for idl in temp:
            sum_layer_weight[idl, 2*idl] = -0.5
            sum_layer_weight[idl, 2*idl+1] = 0.5 
        if candi_tot % 2 == 1:  sum_layer_weight[-1][-1] = 1
        sum_layer.weight = Parameter(sum_layer_weight, requires_grad=False)
        sum_layer.bias = Parameter(sum_layer_bias, requires_grad=False)
        layers.append(sum_layer)
...
def new_add_properties(model, true_label, lb_abs = -1000, ret_ls = False, domain=None, max_solver_batch=10000):
    '''
    ...
    max_pool_layers = new_max_pool(diff_out)
